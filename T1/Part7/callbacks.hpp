	...
	for(auto t = displayFile->getHead(); t != nullptr; t = t->getProximo()) {
		if(t->getInfo()->getId() == id) {
			switch (t->getInfo()->getType()) {
				...
				case TCURVE_BEZIER: {
					BezierCurve *bzc = dynamic_cast<BezierCurve*>(t->getInfo());
					Coordinate c1 = bzc->getP1();
					Coordinate c2 = bzc->getP2();
					Coordinate c3 = bzc->getP3();
					Coordinate c4 = bzc->getP4();

					centerX = ((c2.getX() + c1.getX() + c3.getX() + c4.getX())/4);
					centerY = ((c2.getY() + c1.getY() + c3.getX() + c4.getX())/4);

					rotate(centerX, centerY, bzc, radians);
					return;
				}
				default:
...
void c_translateObject() {
	...
	switch (o->getType()) {
		...
		case TCURVE_BEZIER: {
			...
			Coordinate newP1(oldP1.getX() + dx, oldP1.getY() + dy);
			Coordinate newP2(oldP2.getX() + dx, oldP2.getY() + dy);
			Coordinate newP3(oldP3.getX() + dx, oldP3.getY() + dy);
			Coordinate newP4(oldP4.getX() + dx, oldP4.getY() + dy);
...
void c_scaleObject() {
	...
	switch (o->getType()) {
		...
		case TCURVE_BEZIER: {
			...
			double centerX = ((c2.getX() + c1.getX() + c3.getX() + c4.getX())/4);
			double centerY = ((c2.getY() + c1.getY() + c3.getX() + c4.getX())/4);
